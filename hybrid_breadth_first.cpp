...
bool HBF::compare_heuristics(const HBF::maze_s &a, const HBF::maze_s &b)
{
    return a.f < b.f;
}
HBF::maze_path HBF::search(vector< vector<int> > grid, vector<double> start, vector<int> goal) {
  ...
  */
//Generate heuristics grid
vector< vector<int> > heuristics(grid[0].size(), vector<int>(grid[0].size()));
  for (int i = 0;i<grid[0].size();i++){
    for (int j=0;j<grid.size();j++){
      float x_distance = i - goal[0];
      float y_distance = j - goal[1];
      float euclid_dist = sqrt(x_distance*x_distance + y_distance * y_distance);
      heuristics[i][j] = euclid_dist;
    }
  }
  //Display heuristics grid
  for (int i=0; i<heuristics.size();i++){
    for (int j=0; j<heuristics[i].size();j++){
      cout << heuristics[i][j] << ",";
    }
    cout << endl;
  }
